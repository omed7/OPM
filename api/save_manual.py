...
def load_match_database():
    """
    Loads match database from Supabase.
    Returns (database_list, None) since SHA is no longer needed.
    """
    data, error = supabase_request("/matches?select=*", method="GET")
    if error:
        print(f"Error fetching matches from Supabase: {error}")
        return [], None
    return data or [], None

def save_match_database(new_records):
    """
    Saves new matches to Supabase table via bulk insert.
    """
    if not new_records:
        return True, {}

    data, error = supabase_request("/matches", method="POST", data=new_records)
    if error:
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            database, _ = load_match_database()
            ...
            save_info = {}
            if new_records:
                success, save_info = save_match_database(new_records)
