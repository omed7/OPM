...
- **OddAlerts Automated Data**: OddAlerts.com. Free pages only — robots.txt blocks `/UpdateLiveFeed`, `/UpdateLiveStats`, `/app/`; everything else public is fair game. "Recent Results with xG" on `/xg/<league>` pages is server-rendered and scrapeable. "Upcoming Fixtures" on `/leagues/<country>/<league>/fixtures` pages are server-rendered and scrapeable. 14 valid leagues (including MLS, Eliteserien, Premiership, and Superliga Denmark) are fully automated. Note: Iceland and Canada do not have scrapeable server-rendered fixtures.
- **TheStatsAPI historical dry run (14 OddAlerts leagues)**: `src/dry_run_thestatsapi.py --output <path>` is read-only. It resolves explicit provider competitions/seasons, paginates finished matches, and reports candidates before the approved `2026-08-10` cutoff; it does not call OPM persistence or modify public artifacts. The planned historical source is TheStatsAPI for dates before the cutoff and OddAlerts for dates on/after it. Any production replacement requires a separate reviewed and explicitly approved operation.
- **TheStatsAPI historical import (14 OddAlerts leagues)**: `src/historical_import.py --manifest <reviewed-json> --output <non-public-json>` validates an explicitly reviewed fixture manifest and writes only a local summary by default. Its `--write` mode is deliberately locked behind a separate acknowledgement and must not be run without the user's separately approved production-write scope. It reads existing match natural keys before persistence and retains only new rows; manual xG and approved score overrides remain manifest-only inputs, never browser data.
